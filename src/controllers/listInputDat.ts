...
export const getListInputDats = async (
	req: Request,
	res: Response
): Promise<void> => {
	try {
...
export const createListInputDat = async (
	req: Request,
	res: Response
): Promise<void> => {
	try {
...
export const updateListInputDat = async (
	req: Request,
	res: Response
): Promise<void> => {
	try {
		const { id } = req.params;
		const listInputDat = await ListInputDatsModel.findByIdAndUpdate(
			id,
			req.body
		);
...
export const deleteListInputDat = async (
	req: Request,
	res: Response
): Promise<void> => {
	try {
...
export const getListInputDatsByIndicator = async (
	req: Request,
	res: Response
): Promise<void> => {
	try {
...
export const getEcoequivalences = async (
	req: Request,
	res: Response
): Promise<void> => {
	try {
		...
		if (!listInputDats.length) {
			res.status(200).json({
				message:
					"No se encontraron indicadores para la subcategoría especificada",
				data: [],
			});
			return;
		}
		...
		for (const listInputDat of listInputDats) {
			...
			totalEcoequivalences.co2 +=
				sum * (listInputDat.ecoequivalence?.co2 || 0);
			totalEcoequivalences.agua +=
				sum * (listInputDat.ecoequivalence?.agua || 0);
			totalEcoequivalences.arboles +=
				sum * (listInputDat.ecoequivalence?.arboles || 0);
			totalEcoequivalences.energia +=
				sum * (listInputDat.ecoequivalence?.energia || 0);
		}
		...
		const detailedResponse = {
			...
			indicators: listInputDats.map((item) => ({
				...
				calculatedEcoequivalences: {
					co2:
						(sumByListInputDat[item._id.toString()] || 0) *
						(item.ecoequivalence?.co2 || 0),
					agua:
						(sumByListInputDat[item._id.toString()] || 0) *
						(item.ecoequivalence?.agua || 0),
					arboles:
						(sumByListInputDat[item._id.toString()] || 0) *
						(item.ecoequivalence?.arboles || 0),
					energia:
						(sumByListInputDat[item._id.toString()] || 0) *
						(item.ecoequivalence?.energia || 0),
				},
			})),
...
export const generateExcelTemplate = async (
	...
	try {
		...
		if (!userSelections || !userSelections.selections) {
			res.status(404).json({
				message: "No se encontraron selecciones para este usuario",
			});
			return;
		}
		...
		userSelections.selections.forEach(
			(categoryMap: any, category: string) => {
				if (categoryMap instanceof Map) {
					categoryMap.forEach(
						(indicatorData: any, indicatorId: string) => {
		...
		if (selectedIndicatorIds.length === 0) {
			res.status(404).json({
				message: "No se encontraron indicadores seleccionados",
			});
			return;
		}
		...
		const filteredIds = branchInputDats.filter(
		...
		if (!listInputDats.length) {
			res.status(404).json({
				message:
					"No se encontraron datos de entrada para las selecciones de este usuario",
			});
			return;
		}
		...
		titleCell.value = `CircularIA - Plantilla Personalizada de Datos de Entrada - ${
		...
		instructionCell.value =
			"Descarga la plantilla para rellenar los datos correctamente";
				...
				dataRow.eachCell(
					...
					(cell: any, colNumber: number) => {
						...
						cell.fill = {
							...
							pattern: "solid",
							fgColor: {
								argb: rowIndex % 2 === 0 ? "F5F5F5" : "FFFFFF",
							}, // Gris claro/blanco alternado
						};
		...
		res.setHeader(
			"Content-Disposition",
			`attachment; filename=datos_entrada_${branchId}_${
